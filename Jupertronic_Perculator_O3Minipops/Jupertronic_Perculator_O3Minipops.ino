...
void setup() {
  ...
  pinMode(16, OUTPUT);      //Last step indicator
    ...
    if (RingCount < 255) { //if space in ringbuffer
      ...
      if (!digitalReadFast(10) && ( !extclock || ( !clockstate && clockstateold ))) {
        if (!(tempocnt--)) {
          ...
          if (stepcnt == patlength) digitalWriteFast(16, HIGH); //Send pulse on last step of pattern
          if (stepcnt != patlength) digitalWriteFast(16, LOW); //Not at end of pattern
          if (trig & 1) {
            samplepntQU = 0;
            samplecntQU = 7712;
          }
